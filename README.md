# milledesk
Created for recruitment process task

How to start locally
Backend:
1. Open dotnet/Milledesk.Backend.sln
2. Make sure that startup project is Milledesk.Api
3. Start from visual studio by clicking Debug -> Start Debugging

Frontend:
1. Go to react folder
2. Open anhy terminal and type "npm install"
3. after install type "npm run dev"

backend is running on port 7259 and frontend on 5173


Additional info that i didnt manage to do (run out of time):
1. I would ideally create one more project in .net for Unit tests
2. Same for frontend I would create .spec.ts files for each file
3. I would also add some polcies in repo for PR creating also as CI builds
4. I would also add docker and docker-compose files to run solutions containereized
